...
export type User = {
  id: string; phone_number: string; full_name: string;
  role: "passenger" | "driver" | "admin"; is_active: boolean; created_at: string;
};
export type Driver = {
  user_id: string; full_name: string; phone_number: string; vehicle_plate: string;
  total_earnings: number; is_verified: boolean; rating_avg: number;
  rating_count: number; qr_code: string; created_at: string;
};
export type Transaction = {
  id: string; reference: string; type: "topup" | "payment" | "withdrawal";
  status: "completed" | "pending" | "failed"; amount: number; currency: string;
  sender_id: string | null; receiver_id: string | null; note?: string;
  created_at: string; sender_name?: string; receiver_name?: string;
};
export type Withdrawal = {
  id: string; user_id: string; amount: number; bank_name: string;
  account_number: string; account_name?: string;
  status: "pending" | "approved" | "rejected"; created_at: string; user_name?: string;
};
export type PayoutAccount = {
  id: string; user_id: string; driver_name?: string; type: "self" | "owner";
  bank_name: string; account_number: string; account_name?: string; created_at: string;
};
export type DashboardStats = {
  total_users: number; total_drivers: number; total_transactions: number;
  total_revenue: number; recent_transactions: Transaction[];
};
