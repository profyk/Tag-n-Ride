...
function statusTone(s: string) {
  return s === "completed" ? "green" : s === "pending" ? "yellow" : "red";
}
function typeTone(t: string) {
  return t === "topup" ? "cyan" : t === "payment" ? "green" : "purple";
}
      ...
      {isLoading || !data ? <Spinner /> : (
        <div className="space-y-6">
          ...
          <div>
            ...
            <Table headers={["Reference", "Type", "Amount", "Status", "Date"]} empty={!data.recent_transactions?.length}>
              {data.recent_transactions?.map((t) => (
                <Tr key={t.id}>
                  <Td><span className="font-mono text-xs text-textMuted">{t.reference}</span></Td>
                  <Td><Badge label={t.type} tone={typeTone(t.type) as "cyan" | "green" | "purple"} /></Td>
                  <Td className="font-bold">{formatZAR(t.amount)}</Td>
                  <Td><Badge label={t.status} tone={statusTone(t.status) as "green" | "yellow" | "red"} /></Td>
