          ...
          <>
            <div>
              <div className="flex items-center gap-3 mb-3">
                ...
                {pending.length > 0 && (
                  <span className="bg-yellowDim text-yellow text-xs font-bold px-2 py-0.5 rounded border border-yellow/20">
                    {pending.length}
                  </span>
                )}
              </div>
                  ...
                  <Tr key={w.id}>
                    ...
                    <Td>
                      <div className="flex items-center gap-2">
                        <Button variant="secondary" loading={action.isPending}
                          onClick={() => action.mutate({ id: w.id, action: "approve" })}>
                          <CheckCircle size={14} className="text-green" /> Approve
                        </Button>
                        <Button variant="danger" loading={action.isPending}
                          onClick={() => action.mutate({ id: w.id, action: "reject" })}>
