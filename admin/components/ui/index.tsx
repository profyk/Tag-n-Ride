...
export function Badge({ label, tone }: { label: string; tone: BadgeTone }) {
  return (
    <span className={cn("inline-flex items-center px-2 py-0.5 rounded text-xs font-bold border uppercase tracking-wide", badgeMap[tone])}>
      {label}
    </span>
  );
}
...
export function StatCard({ label, value, sub, tone = "cyan" }: {
  label: string;
  value: string | number;
  sub?: string;
    ...
    <div className="overflow-x-auto rounded-lg border border-border">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-border">
          {empty
            ? <tr><td colSpan={headers.length} className="px-4 py-12 text-center text-textMuted">No records found</td></tr>
            : children}
        </tbody>
      </table>
    </div>
...
export function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input {...props}
      className={cn("w-full bg-bg border border-border rounded-md px-3 py-2 text-text text-sm placeholder:text-textDim focus:outline-none focus:border-cyan transition-colors", props.className)} />
  );
}
...
export function Select({ children, ...props }: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return (
    <select {...props}
      className={cn("bg-bg border border-border rounded-md px-3 py-2 text-text text-sm focus:outline-none focus:border-cyan transition-colors", props.className)}>
      {children}
    </select>
  );
}
