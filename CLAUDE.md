...
```
The app will crash on startup if any Cloudinary var is missing.
...
Key DB tables: `users`, `wallets`, `transactions`, `driver_qr_codes`, `withdrawal_requests`, `kyc_submissions`, `payout_settings`, `system_config`, `cashup_records`, `outstanding_balances`, `owner_drivers`, `fleet_owners`, `payslips`, `statement_requests`, `payroll_runs`, `hr_staff`.
