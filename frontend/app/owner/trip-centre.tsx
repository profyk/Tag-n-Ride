...
export default function OwnerTripCentre() {
  const { state } = useAuth();
  ...
  const load = useCallback(async () => {
    try {
      const [activeRes, histRes, w, txns] = await Promise.all([
        api.tripsActive(),
        api.tripsHistory(),
        api.wallet(),
        api.transactions(),
  ...
  const handleEndTripPress = () => setEndConfirmModal(true);
  ...
  const doEndTrip = async () => {
    ...
    try {
      ...
      try {
        ...
        if (loc?.coords) { lat = loc.coords.latitude; lng = loc.coords.longitude; }
      } catch {}
      ...
      Alert.alert("Could not end trip", e?.message || "Please try again.");
    ...
    <SafeAreaView style={s.root} edges={["top"]}>
      ...
      <View style={s.header}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={s.subtitle}>Drive mode · manage your trips</Text>
        </View>
        ...
        {loading ? (
          ...
          <View style={[s.activeTripSection, { borderLeftColor: colors.green }]}>
            ...
            <View style={s.taxiInfoCard}>
              ...
              {taxiCapacity > 0 && (
                <View style={{ marginTop: 10 }}>
                  ...
                  <Text style={{ color: colors.textDim, fontSize: 10, marginTop: 3, textAlign: "right" }}>{fillPct}% full</Text>
                </View>
              )}
            </View>
            ...
            <View style={s.actionRow}>
              ...
              <TouchableOpacity style={s.actionBtn} onPress={() => setInfoModal(true)} activeOpacity={0.7}>
      ...
      <Modal visible={endConfirmModal} transparent animationType="slide" onRequestClose={() => setEndConfirmModal(false)}>
        <View style={s.modalOverlay}>
          ...
          <View style={s.endConfirmSheet}>
            ...
            <TouchableOpacity style={s.endConfirmBtn} onPress={doEndTrip} activeOpacity={0.85}>
              <Ionicons name="stop-circle-outline" size={20} color="#fff" />
              <Text style={s.endConfirmBtnText}>Confirm — End Trip</Text>
            </TouchableOpacity>
            <TouchableOpacity style={s.keepGoingBtn} onPress={() => setEndConfirmModal(false)} activeOpacity={0.8}>
        ...
        <View style={s.modalOverlay}>
          <View style={s.doneSheet}>
            ...
            <View style={{ gap: 12, marginTop: 20 }}>
              <TouchableOpacity style={s.doneStartBtn} onPress={handleStartNewTrip} activeOpacity={0.85} disabled={starting}>
