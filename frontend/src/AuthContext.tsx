...
type Ctx = {
  ...
  signUp: (body: { phone_number: string; full_name: string; pin: string; role: "passenger" | "driver" }) => Promise<User>;
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const signUp = useCallback(
    async (body: { phone_number: string; full_name: string; pin: string; role: "passenger" | "driver" }) => {
  ...
  return <AuthCtx.Provider value={{ state, signIn, signUp, signOut, refresh }}>{children}</AuthCtx.Provider>;
};
